    ...
    for line in file:
        if (line.find(approute) ) > -1:
            ...
            try:
                ...
                if 'GET' in m :
                  yaml_dict['paths'][pathis]['get'] = {'description':'tst'}
                if 'POST' in m :
                  yaml_dict['paths'][pathis]['post'] = {'description':'tst'}
                if 'DELET' in m :
                  yaml_dict['paths'][pathis]['delete'] = {'description':'tst'} 
            except:
                pathis = path.replace('\"','')   
                yaml_dict['paths'][pathis] = {}
                yaml_dict['paths'][pathis]['get'] = {'description':'tst'}
        elif ('def' in line) and ( pathis != None ):
            yaml_dict['paths'][pathis]['description'] = line.replace("\n","")

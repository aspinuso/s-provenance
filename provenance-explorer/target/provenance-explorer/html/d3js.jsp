...
d3.json(PROV_SERVICE_BASEURL + "workflow/connections/"+RUN_ID+"?level="+RAD_MODE, function(error, classes) {
  ...
  nodes = cluster.nodes(packageHierarchyInstances(classes));
...
//    find(d.name, d);
    var node = d 
  	node.parent=root
    node.parent.children.push(node)
    node.key=d.name.substring(0,20)
...
function packageHierarchyInstances(classes) {
 var map = {};
 var parent
 var root = {name: 'process', children: []};
 classes.forEach(function(d) {
//    find(d.name, d);
    ...
    if (!map[d.name.worker])
    	{
    	 parent = {name: { name: d.name.worker}, children: []};
    	 parent.parent=root
    	 parent.parent.children.push(parent)
    	 map[d.name.worker]=parent
    	}
    
    var node = d 
  	node.parent= map[d.name.worker]
    node.parent.children.push(node)
...
function packageHierarchyWorkers(classes) {
 var map = {};
 var parent
 var root = {name: 'process', children: []};
 classes.forEach(function(d) {
//    find(d.name, d);
    ...
    var node = d 
  	node.parent= map[d.name.worker]
    node.parent.children.push(node)
    node.key=d.name.name+"-"+d.name.worker
...
function packageHierarchyPid(classes) {
 var map = {};
 var root = {name: 'process', children: []};
 classes.forEach(function(d) {
//    find(d.name, d);
    var node = d 
  	node.parent=root
    node.parent.children.push(node)
    node.key=d.name.name+"-"+d.name.pid
...
function packageConnlistPEs(nodes) {
  ...
  nodes.forEach(function(d) {
    if (d.connlist) d.connlist.forEach(function(i) { 
      //console.log(d.name+" "+i.name)
      connlist.push({source: map[d.name], target: map[i.name]});
    });
  });
...
function packageConnlistWorkers(nodes) {
  var map = {},
      connlist = [];
  ...
  nodes.forEach(function(d) {
    //console.log(d)
    map[d.name.name+"-"+d.name.worker] = d;
  });
  ...
  nodes.forEach(function(d) {
    if (d.connlist) d.connlist.forEach(function(i) { 
      //console.log(d.name.name+" "+i.instanceId)
      connlist.push({source: map[d.name.name+"-"+d.name.worker], target: map[i.name+"-"+i.worker]});
...
function packageConnlistPid(nodes) {
  var map = {},
      connlist = [];

  // Compute a map from name to node.
  nodes.forEach(function(d) {
    map[d.name.name+"-"+d.name.pid] = d;
  });
  ...
  nodes.forEach(function(d) {
    if (d.connlist) d.connlist.forEach(function(i) { 
      console.log(d.name+" "+i.instanceId)
      connlist.push({source: map[d.name.name+"-"+d.name.pid], target: map[i.name+"-"+i.pid]});
    });
  });
